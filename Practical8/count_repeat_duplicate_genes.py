import re
import os
os.chdir('E:/zje/ibmi/ibi1/IBI1_2023-24/Practical8')
...
for line in gene:
    if line.startswith('>'):
    #When the line with "duplication," is found, create a file called "duplicate genes.fa'#and write the first gene name into it.#cnt1 == 0 means this is the first time to find duplication in the line with ">"
        if re.search('duplication;',line)and cnt1 == 0:
            ...
            fout.write('\n')
            cnt1 = 1
            cnt2 = 1#Because the "write" will overwrite the file, we need to use "a" to add the next gene name into the file.#use 'a' to avoid overwriting the file.
...
for line in list_seq:
    if len(re.findall(rep_seq, line))> 0:
        fout2 = open(rep_seq+' duplicate genes.fa','a')
